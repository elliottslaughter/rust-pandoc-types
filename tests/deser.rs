...
fn deser_same(src: &[u8]) {
    ...
    let output = p.wait_with_output().expect("Output err");
    let json = String::from_utf8_lossy(&output.stdout);
    let pandoc: Pandoc = serde_json::from_str(&json).expect("Error deserializing json");
    let ser = serde_json::to_string(&pandoc).expect("Error serializing");
    ...
    let ser_output = cmd.wait_with_output().expect("Output err");
    ...
    let orig_output = cmd.wait_with_output().expect("Output err");
